...
LEVEL_COUNT_LABELS = ("3F4", "3H5", "3H4", "3F3+")
...
def format_level_count_summary(level_counts: dict[str, int]) -> str:
    ...
    return (
        "3F4/3H5/3H4/3F3+ = "
        f"{counts['3F4']}/{counts['3H5']}/{counts['3H4']}/{counts['3F3+']}"
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Rendering style: keep the original n2-only view or group states into 3F4 (yellow), 3H4 (red), and 3F3+ (purple) highlight layers.",
    )
    parser.add_argument(
        "--show-n3",
        action="store_true",
